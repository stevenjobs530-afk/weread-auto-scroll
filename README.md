...
打开一本书，页面左侧会出现 **48 像素宽**的进度细条。按 **回车键（Enter）**或点击 **▶** 开始滚动；点击百分比、档位或 **⚙** 展开设置，可查看完整中文说明。默认暂停、默认收起。
...
A、D 每按一次调整一档，长按不连发。当前 5 档时，按 A 的结果是 4、3、2、1；到 1 后不再减小。W、S 暂不绑定功能。调速不改变滚动状态，尚未确认的数字会先应用再加减。输入框、输入法组词和修饰键组合仍不触发这些快捷键。
...
- **键盘没反应：** 确认已更新到 1.4.1，刷新书页，再点击书页空白处。
